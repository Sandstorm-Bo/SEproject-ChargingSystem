...
| `console.html` 运维控制台 | **管理员客户端 · 操控** | 管理员登录 | 启停桩、故障注入/恢复、手动指派、计费策略、倍速/时钟、重置 |
...
# 3. 浏览器打开（双击或 open）
open client.html             # 用户客户端：注册/登录后使用（多标签=多用户）
...
**典型验收流程**：⑥ 设 `10×` + 锚定 06:00 → ⑧ 重置清场 → 在 `client.html` 按脚本提交申请 → 到点用 ①/② 注入故障看重排 → ③ 验证批量 SPT → ⑧ 重置进入下一轮。
